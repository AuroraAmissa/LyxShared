...
cp -v RulebookShared/scripts/repo_files/gitignore .gitignore || exit 1

echo "Updating flake.nix..." || exit 1
cp -v RulebookShared/scripts/repo_files/flake.nix.template flake.nix || exit 1
nix flake update || exit 1

echo "Updating Github Actions"
mkdir -pv .github/workflows || exit 1
cp -v RulebookShared/scripts/repo_files/build_dist.yml .github/workflows/build_dist.yml || exit 1
